...
class World(typing.NamedTuple):
    ...
    @classmethod
    def load(cls, infile: typing.BinaryIO, *, process_voxels: bool = True, process_entities: bool = True) -> "World":
        maps: typing.Dict[str, Map] = {}
        layers: typing.Dict[Vector2IntXZ, ChunkLayer] = {}
        ...
        with io.BytesIO(raw_world.data_and_maps) as world_stream:
            ...
            with zipfile.ZipFile(world_stream) as world_archive:
                    ...
                    if match:
                        ...
                        with world_archive.open(file) as world_data_stream:
                            ...
                            for line in raw_wd.warp_points.split("\r\n"):
                                ...
                                p1 = rp1.split(",")
                                p2 = rp2.split(",")
                                coords[i] = WarpPoint(i, Vector3Int(*(int(p) for p in p1)), Vector3Int(*(int(p) for p in p2)))
                                ...
                                with io.BytesIO(raw_layer.layer) as layer_stream:
                                    ...
                                    with zipfile.ZipFile(layer_stream) as layer_archive:
                                        for file in layer_archive.namelist():
                                            match = re.match(r"^y(\-?\d+)(e?)\.dat$", file)
                                            if not match:
                                                print(f"warning: found odd file {file} in layer {raw_layer.x}, {raw_layer.z}")
                                                continue
                ...
                for file in world_archive.namelist():
                    match = re.match(r"^map/map_([^/]+)/region_(\-?\d+)_(\-?\d+).dat$", file)
                    if match:
                        ...
                        with world_archive.open(file) as region_stream:
                            maps[name].regions[Vector2IntXZ(x, z)] = MapRegion.from_io(region_stream)
        ...
        return World(
            ...
            WorldBounds(
                int(metadata_kvs["Bounds.Left"]) if "Bounds.Left" in metadata_kvs else None,
                int(metadata_kvs["Bounds.Right"]) if "Bounds.Right" in metadata_kvs else None,
                int(metadata_kvs["Bounds.Top"]) if "Bounds.Top" in metadata_kvs else None,
                int(metadata_kvs["Bounds.Bottom"]) if "Bounds.Bottom" in metadata_kvs else None,
            ),
            Vector3(
...
class Chunk:
    ...
    def __init__(self, y: int, voxels: typing.Optional[Voxels] = None, entities: typing.Optional[Entities] = None) -> None:
        self.y = y
        self.voxels = voxels
...
def visualize_world_map(content_path: str, world_name: str) -> PIL.Image.Image:
    ...
    for i in range(256):
        ...
        for x in range(voxel_size):
            for y in range(voxel_size):
                (r, g, b, a) = voxel_altas.getpixel((voxel_u*voxel_size+x, voxel_v*voxel_size+y))
                total_color[0] += r
                total_color[1] += g
                total_color[2] += b
    ...
    for map in world.maps.values():
        ...
        draw.text([(x+x+sizex)/2, (y+y+sizey)/2], mapname(map.name), anchor="mm", font_size=12, fill="#ffffffff")
